    ...
    if os.path.exists(savename) and not overwrite:
        ...
        while page_number < n_page_max:
            ...
            for r, row in enumerate(rows):
                ...
                obj, jd, mag, mag_err, filt, observer = entries[2], entries[3], entries[5], entries[6], entries[7], entries[8]
                try:
                    mag = float(mag)
                except:
                    mag = np.nan
                ...
                df.loc[index, ['obj', 'mjd', 'mag', 'mag_err', 'filter', 'observer']] = obj, float(jd)-2400000.5, float(mag), float(mag_err), filt, observer
            page_number += 1
            pbar.update(1)
            if len(df.index) == 0:
                break
        ...
        lc.query('filter not in @bad_filters', inplace = True) # Drop irrelevant filters
        lc['source'] = 'aavso'
        lc['flag'] = 0
        # print('aavso')
